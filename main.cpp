...
#pragma endregion
	...
	// 定数バッファ用データ構造体(マテリアル)
	struct ConstBuffaerDataMaterial
	{
	...
	cbResourceDesc.Width = (sizeof(ConstBuffaerDataMaterial) + 0xff);
	...
	// 定数バッファのマッピング
	ConstBuffaerDataMaterial* constMapMaterial = nullptr;
...
// ゲームループ
	while (true) {
		...
		{
			FLOAT clearColor[] = { 0.0f,0.55f, 0.5f,0.0f };
			commandList->ClearRenderTargetView(rtvHandle, clearColor, 0, nullptr);
		}
